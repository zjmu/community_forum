package com.jmu.client.module.user.service.impl;

import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.jmu.client.module.article.controller.ArticleController;
import com.jmu.client.module.articleComment.mapper.ArticleCommentExtMapper;
...
import com.jmu.client.enums.UserTypeEnum;
...
public class UserServiceImpl implements UserService {
     ...
     */
    @Override
    public UserVO getUser(UserInfo user) {
    ...
    public Result<String> loginWithOpenId(String openId, LoginReq loginReq) {
        UserInfo userInfo = userMapperExt.getUserByOpenId(openId);
        if (userInfo == null) {
            User user = User.builder()
                    .createTime(JodaUtils.getCurrentDate())
            ...
            userInfo.setNickname(loginReq.getNick());
            userInfo.setOpenId(openId);
            userInfo.setId(user.getId());
        }
        String token = JWTUtil.sign(openId);
        redisUtil.set(token,userInfo,7200);
     ...
     */
    @Override
    public List<UserVO> listSameArea(UserInfo userInfo) {
     ...
     */
    @Override
    public List<UserVO> listSameUnit(UserInfo userInfo) {
        RoomDTO roomDTO = userMapperExt.getRoomInfoOfUser(userInfo.getId());
        Map<String, Object> conditions = new HashMap<>();
        conditions.put("area", roomDTO.getArea());
     ...
     */
    @Override
    public List<UserVO> listSameBuilding(UserInfo userInfo) {
        RoomDTO roomDTO = userMapperExt.getRoomInfoOfUser(userInfo.getId());
        Map<String, Object> conditions = new HashMap<>();
        conditions.put("area", roomDTO.getArea());
        conditions.put("building", roomDTO.getBuilding());
     ...
     */
    @Override
    public List<UserVO> listSameFloor(UserInfo userInfo) {
        RoomDTO roomDTO = userMapperExt.getRoomInfoOfUser(userInfo.getId());
        Map<String, Object> conditions = new HashMap<>();
        conditions.put("area", roomDTO.getArea());
        conditions.put("building", roomDTO.getBuilding());
    ...
    @Override
    @Transactional
    public Result<String> update(UserReq userReq,UserInfo userInfo) {
